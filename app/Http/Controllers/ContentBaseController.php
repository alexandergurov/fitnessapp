...
class ContentBaseController extends VoyagerBaseController {
                ...
                foreach (array_keys($searchNames) as $filter) {
                    if ($filter_value = $request->get($filter)) {
                        ...
                        if ($this->searchFields[$filter]['action'] == 'like') {
                            ...
                            $query->where($table . '.' . $filter, $this->searchFields[$filter]['action'], $filter_value);
                        } else {
                            $query->where($filter, $this->searchFields[$filter]['action'], $filter_value);
                        }
                    }
    ...
    public function exportCsv(Request $request, $slug)
    {
        ...
        $rows = $dataType->browseRows;
        $fileName = $slug . '.csv';
        $items = ContentBaseController::getData($request, $slug);
                ...
                foreach ($rows as $row) {
                    if ($row->type == 'timestamp') {
                        ...
                        switch ($row->getAttribute('details')->table) {
                            case 'instructors':
                                if ($entity = $item->instructor) {
                                    $datarow[$row->display_name] = $entity->{$label};
                                }
                                break;
                            case 'badges':
                                if ($entities = $item->badges) {
                                    $datarow[$row->display_name] = '';
                                    foreach ($entities as $entity) {
                                        $datarow[$row->display_name] .= $entity->{$label} . ', ';
                                    }
                                    $datarow[$row->display_name] = substr($datarow[$row->display_name], 0, -2);
                                }
                                break;
                            case 'users':
                                if ($entity = $item->user) {
                                    if ($entity->name) {
                                        $datarow[$row->display_name] = $entity->name;
                                    } else {
                                        $datarow[$row->display_name] = $entity->email;
                                    }
                                }
                                break;
                        }
