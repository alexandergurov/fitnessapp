...
class AuthController extends Controller
{
    ...
    protected function createNewToken($token){
        return response()->json([
            'access_token' => $token,
            'token_type' => 'bearer',
            'expires_in' => 60*60*24*28,
            'user' => auth('api')->user()
        ]);
    }

    static function clearResetTokens() {
        DB::table('reset_codes')
          ->where('created_at', '<', date('Y-m-d H:i:s', strtotime('-15 minutes')))
          ->delete();
    }

    static function adminLoginRedirect() {
        $user = auth()->guard('web')->user();
        if ($user && $user->hasPermission('browse_admin')) {
            return redirect('/admin');
        } elseif ($user) {
            return view('denied',['user'=>$user]);
        } else {
